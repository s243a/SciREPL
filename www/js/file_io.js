...
class FileIO {
    ...
    /**
     * Whether a language is shipped by the active build profile.
     * Reads window.KERNEL_CONFIG (generated by the build); a language is
     * available unless the config explicitly disables it. Read lazily — the
     * config script loads after file_io.js, so never cache this at construction.
     */
    ...
    /**
     * LANGUAGE_META filtered to languages the active build profile ships.
     * This is the base for every language dropdown/modal, so a profile like
     * `mini` removes its disabled languages from the UI entirely.
     */
    ...
    /**
     * Get the set of enabled language IDs from localStorage, intersected with
     * the languages the build profile actually ships. Defaults to all
     * profile-available languages.
     */
    _getEnabledLanguages() {
        const available = new Set(this._availableLanguageMeta().map(l => l.id));
        const stored = localStorage.getItem('scirepl_enabled_languages');
        if (stored) {
            try {
                const set = new Set(JSON.parse(stored));
                // Drop anything the current profile no longer ships.
                for (const id of [...set]) { if (!available.has(id)) set.delete(id); }
    ...
    _populateLanguagesModal() {
        ...
        for (const lang of this._availableLanguageMeta()) {
    ...
    _rebuildLanguageDropdowns() {
        const enabled = this._getEnabledLanguages();
        const meta = this._availableLanguageMeta().filter(l => enabled.has(l.id));
