...
- replace the remote Ko-fi widget with a locally rendered ordinary link;
...
The current build downloads language runtimes. Before an F-Droid submission:

- replace every rolling URL, including `npm-swipl-wasm/3/latest`, with an immutable version or revision;
- record SHA-256 hashes for every downloaded artifact;
...
Add a human-readable `THIRD_PARTY_NOTICES.md` and, preferably, a machine-readable artifact lock or software bill of materials.
